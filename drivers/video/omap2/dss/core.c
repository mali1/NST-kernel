...
static int omap_dss_setup_clut(struct omap_dss_device *dssdev, dma_addr_t *phy_addr)
{
	...
	if (dssdev->clut_size && dssdev->clut_fill) {
		core.clut_virt = dma_alloc_coherent(NULL, dssdev->clut_size, phy_addr, GFP_KERNEL | GFP_DMA);
		if (core.clut_virt == NULL) {
			printk(KERN_ERR "omapdss: can't allocate memory\n");
			return -ENOMEM;
		}

		/* call the filler */
		return dssdev->clut_fill(core.clut_virt, dssdev->clut_size);
	}
